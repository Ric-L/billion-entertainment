...
export default function VideoCarousel() {
	...
	const getVideoUrl = (videoId: string, startTime: number) => {
		return `https://www.youtube.com/embed/${videoId}?start=${startTime}&autoplay=1&mute=1&loop=1&playlist=${videoId}&controls=0&rel=0&iv_load_policy=3&modestbranding=1&playsinline=1&disablekb=1&fs=0`;
	};
		...
		<section className="relative h-screen w-full overflow-hidden">
			...
			<div className="absolute inset-0">
				...
				<div key={videos[currentIndex].id} className="absolute inset-0 transition-opacity duration-1000 opacity-100">
					<div className="video-bg">
						<iframe
							src={getVideoUrl(videos[currentIndex].id, videos[currentIndex].startTime)}
				...
				{prevIndex !== null && (
					<div key={videos[prevIndex].id} className="absolute inset-0 transition-opacity duration-1000 opacity-0">
						<div className="video-bg">
							<iframe
								src={getVideoUrl(videos[prevIndex].id, videos[prevIndex].startTime)}
								allow="autoplay; encrypted-media; fullscreen; picture-in-picture"
								allowFullScreen
								referrerPolicy="no-referrer-when-downgrade"
								style={{
			...
			<div className="absolute top-4 right-4 sm:top-8 sm:right-8 z-30 flex items-center space-x-2 sm:space-x-4">
				<a
					...
					className="p-2 sm:p-3 rounded-full hover:bg-white/20 transition-colors cursor-pointer"
				>
			...
			<div className="absolute bottom-6 sm:bottom-8 left-1/2 transform -translate-x-1/2 z-20">
				<div className="flex items-center gap-3 sm:gap-4">
					<Button
						...
						size="icon"
						onClick={prevVideo}
						className="h-8 w-8 sm:h-9 sm:w-9 bg-white/20 border-white/30 text-white hover:bg-white/30"
					>
